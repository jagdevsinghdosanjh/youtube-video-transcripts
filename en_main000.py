...
if st.button("Submit for Transcription", key="submit_button"):
...
for job in st.session_state.job_queue:
    if job["status"] == "Queued":
        ...
        # except Exception as e:
        #     job["status"] = "Failed"
        #     logging.exception(f"{datetime.now()} - Job {job['id']} failed")
    ...
    if job["status"] == "Ready":
        ...
        if st.button("🌍 Translate Transcript", key=f"translate_button_{job['id']}"):
            try:
                ...
                st.markdown("### 🌍 Translated Transcript ({target_lang.title()})")
                st.markdown(translated_text)

                translated_pdf_path = "jobs/{job['id']}/translated_{target_lang}.pdf"
                save_translated_pdf(translated_text, translated_pdf_path)

                with open(translated_pdf_path, "rb") as f:
                    st.download_button(
                        label="📥 Download {target_lang.title()} Transcript PDF",
                        data=f.read(),
