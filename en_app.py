...
def plot_keyword_frequency(text, keywords):
    counts = Counter(word for word in text.lower().split() if word in keywords)
    fig, ax = plt.subplots()
...
for job in st.session_state.job_queue:
    if job["status"] == "Queued":
        ...
        with st.spinner(f"Processing Job {job['id']}..."):
            try:
    ...
    if job["status"] == "Ready":
        ...
        if st.button("🌍 Translate Transcript", key=f"translate_button_{job['id']}"):
            try:
                ...
                with col2:
                    st.markdown(f"**Translated ({target_lang.title()})**")
                ...
                fig = plot_keyword_frequency(job["transcript"], ["education", "science", "technology"])
                st.pyplot(fig)
